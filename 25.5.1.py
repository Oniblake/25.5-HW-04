...
def testing():
   ...
   pytest.driver.get('http://petfriends1.herokuapp.com/login') # Переход на страницу авторизации
   pytest.driver.find_element_by_id('email').send_keys('oniblake@gmail.com')
   pytest.driver.find_element_by_id('pass').send_keys('20070118091122')
...
def test_my_pets():
   ...
   # Проверка повторяющихся питомцев
   data_list = []
   rows = WebDriverWait(pytest.driver, 10).until(
      EC.presence_of_all_elements_located((By.XPATH,
                                             '//div[@id="all_my_pets"]/table[@class="table table-hover"]/tbody/tr'))
   ...
   # Убираем повторяющиеся элементы путем преобразования списка во множество
   data_set = set(data_list)

   assert len(data_list) == len(data_set)

   # УБедимся, что у питомцев разные имена
   name_list = []
   rows = WebDriverWait(pytest.driver, 10).until(
      EC.presence_of_all_elements_located((By.XPATH,
                                             '//div[@id="all_my_pets"]/table[@class="table table-hover"]/tbody/tr'))
